({
    ...
    /**
     * The temporary "(New Attendee}" string to add if users type in their own product name
     * @type {string}
     */
    createNewLabel: undefined,

    /**
     * The temporary ID to user for newly created Attendee names
     * @type {string}
     */
    newAttendeeId: undefined,

    /**
     * @inheritdoc
     */
    initialize: function(options) {
        this.createNewLabel = app.lang.get('LBL_CREATE_NEW_Attendee_IN_DROPDOWN', 'Contacts');
        this.newAttendeeId = 'newAttendeeId';

        this._super('initialize', [options]);
    },

    /**
     * Overriding because getSearchModule needs to return Contacts for this metadata
     *
     * @inheritdoc
     */
    _getPopulateMetadata: function() {
        return app.metadata.getModule('Contacts');
    },

    /**
     * Overridden select2 change handler for the custom case of being able to add new unlinked Contacts
     * @param evt
     * @private
     */
    _onSelect2Change: function(evt) {
        ...
        if (value && value.indexOf(this.createNewLabel)) {
            // if value had new Attendee label, remove it
        ...
        if (id === this.newAttendeeId) {
            // if this is a new Attendee
            this.model.set({
                name: value
            });
    ...
    /**
     * Use the Contacts module and record ID to build route.
     *
     * @inheritdoc
     */
