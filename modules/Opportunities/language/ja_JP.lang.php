...
// BEGIN ENT/ULT
    ...
    // List View Help Text
    'LBL_HELP_RECORDS' => '{{plural_module_name}}モジュールにより、個別の販売を最初から最後まで追跡することが可能になります。各{{module_name}}レコードは販売見込みを表し、また、関連販売データ、および{{quotes_module}}、{{contacts_module}}などの重要なデータを含むことができます。{{module_name}}は一般的に複数の販売ステージを通じて使用され、最終的に「クローズドウォン」か「クローズドロスト」とマークされます。{{plural_module_name}}では、Sugarの{{forecasts_singular_module}}モジュールにより機能を強化し、販売トレンドの理解や予測、および販売ノルマの達成に集中することができます。',
...
- 左下のウインドウと「アクティビティストリーム」をトグリングすることにより、{{activitystream_singular_module}} 内にユーザーコメントの作成や表示、および変更履歴を記録する。
